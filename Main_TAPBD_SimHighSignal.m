...
axes(subplot_handles(Nw+1+1));    imagesc(angle(GT_turbulence(range_pupil,range_pupil))); axis image; axis off; title ('GT');
for q = 1:length(SLMnum_arr)
axes(subplot_handles(q+1));       imagesc(upd_Orecon(range_img,range_img, q)); axis image; axis off; title(string(SLMnum_arr(q)));
axes(subplot_handles(q+Nw+1+1));    imagesc(angle(upd_turbrecon(range_pupil,range_pupil, q))); axis image; axis off; title(string(SLMnum_arr(q)));
end
